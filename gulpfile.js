/**
 * test Gulp file для формирования шаблонов под CMS MODx с использованием шаблонизатора Fenom.
 *
 ...
 * @version 1.0.16+fix
 *
 * 2020.02.11 добавил base task
 * добавил вывод текущего года в шаблон
 * добавил gulp-image-resize специально для текущего проекта
 */
...
const config = gulpConfig.initialize({
  'domain': domain
});
...
task('sprites:svg_font', series(function () {
    ...
    // config.svgFont.targetPath = process.cwd() + '/xxx.styl';
    // config.svgFont.targetPath = '../../../../tmp/css/svg-font.styl';
    // console.log(config.svgFont);
...
task('assets', function () {
  return src([
      './src/' + domain + '/**',
      '!src/' + domain + '/base/**', // тут храним все от корня сайта
...
// Использует папку template/src/<domain_name>/base в качестве корня папки dist
task('base', function () {
    return src([
            './src/' + domain + '/base/**',
        ], {
            base: 'src/' + domain + '/base/',
            since: lastRun('base')
        })
        .pipe(newer('base')) // после newer лучше сжимать изображения, тогда это делается один раз
        .pipe(dest(config.target_path));
});
...
task('images_min', series(function () {
  // https: //habr.com/ru/post/422531/#04
  let w=218;
  return src([
      `${config.target_path__assets_full}images/**/*.{jpg,png}`,
    ...
    .pipe(rename(function (file) {
      // file.basename = `${file.basename}_w${w}`;
...
task('stylus', function (done) {
  ...
  return src(['tmp/css/**.{css,styl}', './src/' + domain + '/css/**/*.css', './src/' + domain + '/css/*.styl'], // строгая последовательность
      // return src(['tmp/css/**.styl'], // строгая последовательность
      // return src('./src/css/**/*.{css,styl}',
      // return src('./src/css/**/*',
      // {
      // base:'src/css',
      // since:lastRun('stylus')  // ммм не всегда срабатывает с кешированием
      // }
    )
    // .pipe(cached('stylus_cached'))
    ...
    // .pipe(remember('stylus_css')) // добавляет файлы из кеша stylus_css если
    // они не попали в массив из-за since, хранит в себе полный массив src
    ...
    // странно не сжимает css нужно проверить
...
task('combine_production', function (done) {
  ...
  // else{
  // TODO: убрать статичное подключение файлов при отсуствии манифеста
  // если убрать эту строку то вендоры будут подключаться после основныех скриптов
  // таким образом исправляем это хоть это и не правильно!!!
  // excepts = 'prod.js|prod.css|prod.min.css|prod.min.js|scripts.js|style.css';
  // }
...
const nunjucks_compiler = function (done, src_templates = './tmp/templates_prepared', target_path = './tmp/templates_compiled') {
  ...
  function nunjucks_macros_include(path = '', props = {}) {
    ...
    // var result = new nunjucks.precompileString('{% include "sections/header.tpl" %}', {});
  ...
  if (isDevelopment){
      _vars._modx.config.site_dev_mode = isDevelopment & 1;
  }

  logger.info('Используемые переменные в шаблоне', _vars);
  logger.info(_vars);

  let nunjucks_config = _merge(
    {
      // env: nunjucks_env,
    ...
    .pipe(through.obj((file, enc, cb) => {
      ...
      if (!file.isDirectory()) {
        ...
        switch (config.convertor_type) {
          ...
          case 'twig':
          content = content.replace(new RegExp('\{%\\s*endblock\\s*%\}', 'gi'), '{% endblock %}');
          ...
          content = content.replace(new RegExp('\{\{\\s*\\$?include\\s*\\(?\\s*([\'|\"])([^\\1|\\s]+)\\1\\s*\\)?\\s*\}\}', 'gim'), "{{ include ('$2') }}");
          // console.log(content);
          ...
          content = content.replace(new RegExp('\{%\\s*extends\\s*([\'|\"])_?([^\\1|\\s]+)\\1\\s*%\\s*\}', 'gim'), '{% extends "$2" %}')
...
task('watch', function (cb) {
  ...
  watch('./src/' + domain + '/base/**/*', parallel('base')).on('unlink', function (filepath) {
      // удаляем файл из целевой папки
      var filePathFromSrc = path.relative(path.resolve('src/' + domain + '/base'), filepath);
      var destFilePath = path.resolve(config.target_path, filePathFromSrc);
      del.sync(destFilePath);
    //   logger.info('del0=', destFilePath);
  });
;
});
