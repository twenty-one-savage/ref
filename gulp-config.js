 ...
 * Модуль конфигурации.
 *
 * Формирует базовую конфигурацию для работы gulpfile, может быть расширен
 * через размещение файлов в src/[<domain name>]/_config/[<config part name>].json
 * domain name - web и config part name - config|common загружаются по дефолту.
 * Файлы расширения могут отсутствовать.
 * Может запускаться как самостоятельный скрипт для тестирования формирования
  ...
  get_config(part = 'common') {
    ...
    switch (part) {
      ...
      case 'common':
        ...
        // нужно делать предварительное слияние иначе переиспользуемые значения
        // свойств в this.config не будут перезаписаны
        result = _merge(this.config, ext_config);
